...
const charitieUpdateRoute = express.Router();
...
charitieUpdateRoute.post('/updateCharityInfo', upload.fields([{ name: 'profileImage', maxCount: 1 }]), async (req, res) => {
  try {
    ...
    if (onDeletion === true || onDeletion === 'true') {
      try {
        ...
        const getCurrentProfilePathQuery = "SELECT profile_path FROM charities WHERE user_id = ?";
        ...
        const updateQuery = "UPDATE charities SET name = ?, profile_path = ?, pin_code = ?, phone = ?, address = ? WHERE user_id = ?";
      ...
      try {
        const query = "UPDATE charities SET name = ?, pin_code = ?, phone = ?, address = ? WHERE user_id = ?";
...
module.exports = charitieUpdateRoute;