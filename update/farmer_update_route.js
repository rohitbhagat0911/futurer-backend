  ...
  try {
    ...
    if (onDeletion === true || onDeletion === 'true') {
      try {
        ...
        const getCurrentProfilePathQuery = "SELECT profile_path FROM farmers WHERE user_id = ?";
        ...
        const updateQuery = "UPDATE farmers SET name = ?, profile_path = ?, pin_code = ?, phone = ?, address = ? WHERE user_id = ?";
      ...
      try {
        const query = "UPDATE farmers SET name = ?, pin_code = ?, phone = ?, address = ? WHERE user_id = ?";
